...
\fi
	...
	\begin{align*}
	\{a_1,a_2,a_3, \dots\} \text{\text{скорочено}}{=} \{a_n, n \geq 1\}.
	\end{align*}
...
\end{definition}
	...
	\end{align*}
	\begin{align*}
	\forall \{x_n, n \geq 1\}\subset A: \forall n \geq 1: x_n \neq x_0: \lim_{n \to \infty} x_n = x_0 \Rightarrow \lim_{n \to \infty} f(x_n) = b \tag*{означення Коші}
	\end{align*}
	...
	Таким чином, $\displaystyle \lim_{n \to \infty} f(x_n) = b$ - тим самим виконано означення Гейне.
	\bigskip \\
	\leftproof Дано: означення Гейне, або $\displaystyle \forall \{x_n, n \geq 1\}\subset A: x_n \neq x_0: \forall n \geq 1: \lim_{n \to \infty} x_n = x_0 \Rightarrow \lim_{n \to \infty} f(x_n) = b$.\\
...
\end{example}
	...
	За означенням та запереченням Гейне, зафіксуємо наступну послідовність: $\displaystyle \left\{x_n = \frac{(-1)^n}{2n}, n \geq 1\right\}$, де $\displaystyle \lim_{n \to \infty} x_n = 0$. Але $\displaystyle \lim_{n \to \infty} \dfrac{|x_n|}{x_n} = \left[ \begin{gathered} 1, n = 2k \\ -1, n = 2k-1 \end{gathered} \right.$ - не збіжна, бо має різні часткові границі.\\
	...
	\end{align*}
	\begin{align*}
	\forall \{x_n, n \geq 1\}\subset A: \forall n \geq 1: x_n \neq x_0: \lim_{n \to \infty} x_n = x_0 \Rightarrow \lim_{n \to \infty} f(x_n) = \infty \tag*{означення Гейне}
	\end{align*}
	Позначення: $\displaystyle \lim_{x \to x_0} f(x) = \infty$.\\
	Якщо виконується $f(x) > E$, то тоді $\displaystyle \lim_{x \to x_0} f(x) = \infty$.\\
	Якщо виконується $f(x) < -E$, то тоді $\displaystyle \lim_{x \to x_0} f(x) = -\infty$.
	\end{definition}
...
\end{figure}
	...
	\end{align*}
	\begin{align*}
	\forall \{x_n, n \geq 1\} \subset \mathbb{R}: \forall n \geq 1: \lim_{n \to \infty} x_n = \infty \Rightarrow \lim_{n \to \infty} f(x_n) = b \tag*{означення Гейне}
	\end{align*}
	Позначення: $\displaystyle \lim_{x \to \infty} f(x) = b$.\\
	Якщо $x \to +\infty$, то ми вимагаємо $x > \Delta$.\\
	Якщо $x \to -\infty$, то ми вимагаємо $x < \Delta$.
	\end{definition}
	...
	\begin{remark}
	Можна самостійно записати означення Коші та означення Гейне для випадку $\displaystyle \lim_{x \to \infty} f(x) = \infty$.
	\end{remark}
	
	\begin{remark}
	Для інших варіацій границь функції: $\displaystyle\lim_{x \to x_0} f(x) = \infty, \lim_{x \to \infty} f(x) = b, \lim_{x \to \infty} f(x) = \infty$ - еквівалентність двох означень, за Коші та за Гейне, залишається в силі.
	\end{remark}
...
\forall \{x_n,n\geq 1\} \subset A: \forall n \geq 1: x_n > x_0: \lim_{n \to \infty}x_n = x_0 \Rightarrow \lim_{n \to \infty}f(x_n) = b \tag*{означення Гейне}
\end{align*}
...
\forall \{x_n,n\geq 1\} \subset A: \forall n \geq 1: x_n < x_0: \lim_{n \to \infty}x_n = x_0 \Rightarrow \lim_{n \to \infty}f(x_n) = \tilde{b} \tag*{означення Гейне}
\end{align*}
...
\begin{remark}
Для функції $f(x) = \sqrt{x}$ є границя $\displaystyle\lim_{x \to 0+0} \sqrt{x} = 0$, але не існує $\displaystyle\lim_{x \to 0-0} \sqrt{x}$. Тобто не існує $\displaystyle\lim_{x \to 0} \sqrt{x}$ - звісно, ні. Ми маємо $\displaystyle\lim_{x \to 0} \sqrt{x} = 0$.\\
...
$\displaystyle\lim_{x \to 0+0} \dfrac{|x|}{x} = 1 \qquad \lim_{x \to 0-0} \dfrac{|x|}{x} = -1$.
\end{example}
...
\begin{example}
Зокрема $f(x) = \sqrt{x}$ монотонно строго зростає. Дійсно, $x_1 > x_2$, де $x_1,x_2 \in [0,+\infty)$, маємо\\
$f(x_1) - f(x_2) = \sqrt{x_1} - \sqrt{x_2} = \dfrac{x_1-x_2}{\sqrt{x_1} + \sqrt{x_2}} > 0 \implies f(x_1) > f(x_2)$.
\end{example}

\begin{definition}
Функція $f \colon A \to \mathbb{R}$ називається \textbf{обмеженою}, якщо
\begin{align*}
...
\bigskip \\
Отже, $f$ - строго спадає, тобто $\forall x_1,x_2 \in (a,b): x_1>x_2 \Rightarrow f(x_1)<f(x_2)$. У нашому випадку $d = \displaystyle \inf_{x \in (a,b)} f(x)$. У інфімума кілька випадків.
\bigskip \\
...
\begin{example}
Розглянемо декілька прикладів.\\
1. $f(x) = e^{-x}$ - монотонно спадає на $\mathbb{R}$, тому $\displaystyle\lim_{x \to +\infty} e^{-x} = 0, \quad \lim_{x \to -\infty} e^{-x} = +\infty$\\
2. $f(x) = \arctg x$ - монотонно зростає на $\mathbb{R}$, тому $\displaystyle\lim_{x \to +\infty} \arctg x = \dfrac{\pi}{2}, \quad \displaystyle\lim_{x \to -\infty} \arctg x = -\dfrac{\pi}{2}$.
\end{example}
	...
	\begin{proof}
	Зафіксуємо $\{x_n, n \geq 1\}$, таку, що $\displaystyle \lim_{n \to \infty} x_n = x_0$. Тоді за Гейне, $\displaystyle \lim_{n \to \infty} f(x_n) = 0$, $\displaystyle \lim_{n \to \infty} g(x_n) = \infty$, отже:\\
	$\{f(x_n), n \geq 1\}$ - н.м.;\\
	$\{g(x_n), n \geq 1\}$ - н.в.;\\
...
Задано функції $f \colon A \to B$, $g: B \to \mathbb{R}$ та композиція $h = g(f(x))$. Більш того, $x_0, y_0 \in \mathbb{R}$ - граничні точки відповідно для $A,B$ та $\exists \displaystyle \lim_{x \to x_0} f(x) = y_0$ та $\exists \displaystyle \lim_{y \to y_0} g(y) = b$.\\
...
\end{figure}
Виділимо з малюнку наступні дані: $|AB|=\sin x, \quad |AC|=x, \quad |KC|=\tg x$.\\
...
1) \textit{Вказівка: $\tg x = \dfrac{\sin x}{\cos x}$. Необіхдно знати про неперервність $\cos x$. (буде в наступного розділі)} \\
...
Тоді за Ваєрштрасом, для підпослідовності $\{x_{n_{m_p}}, p \geq 1\}: \exists \displaystyle \lim_{n \to \infty} x_{n_{m_p}} =x_*$.\\
...
Ми також маємо обмежену послідовність $\{x_n, n \geq 1\} \subset [a,b]$. Тому за Ваєрштрасом, для послідовності $\{x_{n_k},k \geq 1\}$ існує $\displaystyle \lim_{n \to \infty} x_{n_k} = x_*$. Отже, за Гейне і за неперервністю, $\displaystyle \exists \lim_{k \to \infty} f(x_{n_k}) = f(x_*)$.\\
...
Функція $f$ \textbf{задовільняє умові Ліпшиця}, якщо
\begin{align*}
...
Функція $f: A \to \mathbb{R}$ опукла вниз $\iff$ справедлива нерівність\\
...
\begin{remark}
Для опуклої вгору нерівність навпаки. Для строгої опуклості нерінвість буде строгою.
\end{remark}
...
Задано функцію $f:(a,b) \to \mathbb{R}$ - опукла $\underset{\textrm{вгору}}{\textrm{вниз}}$. Тоді\\
...
$f(\alpha_1 x_2 + \alpha_2 x_2) = f(\alpha_1 x_2 + (1-\alpha_1)x_2) < \alpha_1 f(x_1) + (1-\alpha_1)f(x_2)$, оскільки наша функція опукла вниз.\\
...
IV. $(1+x)^{\alpha} \displaystyle = 1 + \frac{\alpha x}{1!} + \frac{\alpha (\alpha-1) x^2}{2!} + \dots + \frac{\alpha (\alpha-1)\dots(\alpha-(n-1)) x^n}{n!} + o(x^n)$\\
