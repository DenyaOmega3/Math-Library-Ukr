...
Задамо ілюстративним чином операції над векторами:\\
	- \textbf{додавання} (є два способи, яка додавати);
\begin{figure}[H]
\centering
	\begin{tikzpicture}
...
\centering
	...
	\end{tikzpicture}
\end{figure}
	3) $\text{існує $\vec{0}$, так званий нульовий вектор, для якого } \vec{a} + \vec{0} = \vec{a}$; цей нульовий вектор має нульову довжину, тобто $|\vec{0}| = 0$, а геометричним чином представляє собою точку;\\
	4) $\text{існує $\tilde{\vec{a}}$, так званий протилежнй вектор, для якого } \vec{a} + \tilde{\vec{a}} = \vec{0}$; насправді, цей $\tilde{\vec{a}} = -\vec{a}$, тобто це той же самий вектор тільки в протилежному напрямку;\\
	\textit{(для 3), 4), малюнок навряд знадобиться)}
	\bigskip \\
	5) $\lambda(\vec{a}+\vec{b}) = \lambda \vec{a} + \lambda \vec{b}$ \textit{(випливає з подібності двох трикутників)};
\begin{figure}[H]
\centering
...
\begin{proposition} 
Задано базис векторів $\vec{a}$, $\vec{b}$, $\vec{c}$ і вектори $\vec{d}_1 = (\alpha_1, \beta_1, \gamma_1)$, $\vec{d}_2 = (\alpha_2, \beta_2, \gamma_2)$. Тоді:
