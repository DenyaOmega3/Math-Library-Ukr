...
Задана $\{y_1,\dots, y_n\}$ -- фундаментальна система розв'язків рівняння та якась точка $x_0 \in I$. Тоді\\
$W[y_1, \dots, y_n](x) = W[y_1, \dots, y_n](x_0)e^{\displaystyle -\int_{x_0}^x a_{n-1}(t)\,dt}$.
\end{theorem}

\begin{proof}
Знайдемо похідну від визначника Вронського за щойно доведеною формулою:\\
...
\end{vmatrix} \boxed{=}
$\\
Тут всі визначники, окрім останнього, онуляться через однакові рядки\\
\\
$\displaystyle \boxed{=} \begin{vmatrix} 
	y_1(x) &  y_2(x) & \dots & y_n(x) \\ 
	y'_1(x) &  y'_2(x) & \dots & y'_n(x) \\
	\vdots &  \vdots & \ddots & \vdots \\
	y^{(n-2)}_1(x) &  y^{(n-2)}_2(x) & \dots & y^{(n-2)}_n(x) \\ 
	y^{(n)}_1(x) &  y^{(n)}_2(x) & \dots & y^{(n)}_n(x) \\ 
\end{vmatrix} \boxed{\boxed{=}}$\\
Оскільки $y_1, \dots, y_n$ -- розв'язки, то ми можемо виразити старші похідні з лінійного рівняння:\\
$y^{(n)}_i = -a_0(x)y_j - a_1(x)y_j' -\dots - a_{n-1}(x)y_j^{(n-1)},\quad j=\overline{1,n}$.\\
Підставимо в наш останній визначник і зробимо такі кроки:\\
...
$\displaystyle \boxed{\boxed{=}} -a_{n-1}(x)\begin{vmatrix} 
	y_1(x) &  y_2(x) & \dots & y_n(x) \\ 
	y'_1(x) &  y'_2(x) & \dots & y'_n(x) \\
	\vdots &  \vdots & \ddots & \vdots \\
	y^{(n-1)}_1(x) &  y^{(n-1)}_2(x) & \dots & y^{(n-1)}_n(x) \\ 
\end{vmatrix} = -a_{n-1}(x)W[y_1, \dots, y_n](x)$.\\
Отже, отримали таку рівність: $W'[y_1, \dots, y_n](x) = -a_{n-1}(x)W[y_1, \dots, y_n](x)$.\\
А це -- диференціальне рівняння з відокремленими змінними, яку ми розв'яжемо:\\
